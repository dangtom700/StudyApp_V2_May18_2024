...
namespace RECOMMEND
{
    ...
    std::map<std::string, std::map<std::string, double>> load_related_tokens_topic(
        sqlite3 *db,
        const std::vector<std::tuple<std::string, int, double>> &filtered_tokens)
    {
        ...
        const char *insert_sql = "INSERT OR IGNORE INTO temp_tokens (token, frequency, base_distance) VALUES (?, ?, ?);";
        ...
        for (const auto &[token, freq, base_distance] : filtered_tokens)
        {
            sqlite3_reset(insert_stmt);
            sqlite3_bind_text(insert_stmt, 1, token.c_str(), -1, SQLITE_TRANSIENT);
            sqlite3_bind_int(insert_stmt, 2, freq);
            sqlite3_bind_double(insert_stmt, 3, base_distance);
            sqlite3_step(insert_stmt);
        }
        sqlite3_finalize(insert_stmt);

        // 3. Query related tokens via JOIN
        const char *query_sql = "SELECT r.topic, t.token, t.base_distance "
                                "FROM topic_token r "
